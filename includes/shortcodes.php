...
function js_add_perfect_quotes_button($context){
    $image_btn = plugins_url('images/perfect-space-icon.png', dirname(__FILE__));
    $out = '<a href="#TB_inline?width=450&inlineId=insert_perfect_quote" class="thickbox" title="Insert Perfect Quote"><img src="'.$image_btn.'" alt="Insert Perfect Quote" /></a>';
    return $context . $out;
}

add_action('admin_footer','js_add_perfect_quotes_popup');
add_action('media_buttons_context','js_add_perfect_quotes_button');

//Action target that displays the popup to insert a form to a post/page
function js_add_perfect_quotes_popup(){
  ...
  <div id="insert_perfect_quote" class="folded" style="display:none;">
    <div class="wrap">
      <div>
        ...
        <div style="padding:15px 15px 0 15px;">
          <label style="display:block;color:#21759B;font-size:12px;font-weight:bold;padding:0 0 8px;text-shadow:0 1px 0 #FFFFFF;">Choose to display a random quote(s)</label>
        ...
        <div style="padding:15px;">
          <input type="button" class="button-primary" value="Insert into Post" onclick="InsertGallery();"/>&nbsp;&nbsp;&nbsp;
    ...
    <script type="text/javascript">
        ...
        function InsertGallery(){
            ...
            if (quote){
              ...
              win.send_to_editor('[perfect_quotes num="' + number + '"' + random + ']');
            } else if (random) {
              jQuery("#js_perfect_quote_shortcode_random").removeAttr('checked');
              win.send_to_editor('[perfect_quotes' + random + ']');
            } else {
